...
UserManager = function(host, port) {
  ...
  this.db.open(function(){});
	...
	UserManager.prototype.findByUsername = function(usernameArray, callback){
		...
		var that = this;
		var numUsers = 0;
		this.find({"username":{"$in":usernameArray}}, function(error, userInfoArray){
			if(error) callback(error);
			userInfoArray.sort(function(a,b){
				return usernameArray.indexOf(a.username) > usernameArray.indexOf(b.username);
			});
